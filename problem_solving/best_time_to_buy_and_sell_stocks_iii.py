...
def maxProfit(self, prices: List[int]) -> int:
	...
	def recursive(index: int, bought: bool, transactions: int) -> int:
		# base case
		# all 2 transactions have been completed, or I've exhausted looking at the array
		if transactions == 0 or index == len(prices):
			return 0

		# if I've already bought a stock before need to sell
		if bought:
	...
	# return recursive(0, False, 2)

	# recursive memoized top down TC -> O(2 * (k + 1) * (n + 1)) ≈ O(n), SC -> O(2 * (k + 1) * (n + 1) + n (auxiliary)) ≈ O(n), where n = len(prices), k = max_transactions = 2
	...
	# iterative bottom up TC -> O(2 * (k + 1) * (n + 1)) ≈ O(n), SC -> O(2 * (k + 1)) ≈ O(1), where n = len(prices), k = max_transactions = 2
